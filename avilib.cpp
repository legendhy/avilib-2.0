...
avi_t* AVI_open_output_file(char * filename)// 创建avi文件,打开avi文件
{
   avi_t *AVI;
   int i;
...
//width:图像宽度；height：图像高度；fps：帧率；compressor：视频压缩类型，比如"MJPG","H264"
...
// channels：音频通道；rate：采样率；bits：位宽；音频格式；mp3rate：mp3采样率频率，单位kbps
